...
class DexApp(QMainWindow):
    def __init__(self):
        ...
        usb_group = QGroupBox("【USB ADB 接続ステータス】")
        usb_layout = QVBoxLayout()
        
        device_select_layout = QHBoxLayout()
        device_select_layout.addWidget(QLabel("接続中デバイス:"))
        self.combo_devices = QComboBox()
        device_select_layout.addWidget(self.combo_devices)
        
        btn_refresh = QPushButton("手動更新")
        ...
        btn_usb_mode = QPushButton("USBモードに切替 (adb usb)")
        ...
        wifi_group = QGroupBox("【ワイヤレス ADB 接続】")
        wifi_layout = QVBoxLayout()
        
        pair_layout = QHBoxLayout()
        self.input_pair_ip = QLineEdit()
        self.input_pair_ip.setPlaceholderText("IP:Port (ペアリング用)")
        self.input_pair_code = QLineEdit()
        self.input_pair_code.setPlaceholderText("ペアリングコード")
        btn_pair = QPushButton("ペアリング")
        ...
        self.input_conn_ip.setPlaceholderText("IP:Port (接続用 :5555)")
        btn_conn = QPushButton("接続")
        ...
        self.btn_toggle_dex = QPushButton("DeX 起動")
    ...
    def show_license_dialog(self):
        ...
        QMessageBox.about(self, "ライセンス情報", license_text)
    ...
    def update_device_list(self):
        ...
        if not devices:
            self.combo_devices.addItem("デバイスが検出されません")
        else:
            for dev in devices:
        ...
        for dev in devices:
            if dev["serial"] in new_devices:
                if dev["status"] == "unauthorized":
                    self.log(f"⚠️ デバイスが検出されましたが未承認です: {dev['serial']}")
                    self.log("👉 スマホ画面で「USBデバッグを許可」をタップしてください。")
                elif dev["status"] == "device":
                    self.log(f"✅ 接続完了: {dev['model']} ({dev['serial']})")
                else:
                    self.log(f"⚠️ 接続状態 [{dev['status']}]: {dev['serial']}")

        removed_devices = self.known_serials - current_serials
        for serial in removed_devices:
            self.log(f"❌ 切断されました: {serial}")
    ...
    def handle_toggle_dex(self):
        if self.dex.process is None or self.dex.process.poll() is not None:
            dev = self.get_selected_device()
            if not dev or "serial" not in dev:
                self.log("エラー: 有効なデバイスを選択してください。")
                return
            
            if dev["status"] != "device":
                self.log(f"エラー: デバイスが準備完了状態ではありません ({dev['status']})")
                return
            ...
            self.log(f"DeX起動 [{dev['serial']}]: {w}x{h} / {dpi}DPI")
            self.btn_toggle_dex.setText("DeX 停止")
        else:
            self.dex.stop_dex()
            self.log("DeX停止")
            self.btn_toggle_dex.setText("DeX 起動")
